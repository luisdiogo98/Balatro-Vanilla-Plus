...
extern PRECISION vec2 infected;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    //vec2 rotater = vec2(cos(infected.r*0.1221), sin(infected.r*0.3512));

    vec3 infectedVec = tex.rgb;
    if (((tex.r + tex.g + tex.b) < 2.95) && (infected.g > 0.0 || infected.g < 0.0)) {
        infectedVec.r = ((tex.r * 0.393) + (tex.g * 0.769) + (tex.b * 0.189)) * 0.35;
        infectedVec.g = ((tex.r * 0.349) + (tex.g * 0.686) + (tex.b * 0.168)) * 0.7;
        infectedVec.b = ((tex.r * 0.272) + (tex.g * 0.534) + (tex.b * 0.131)) * 0.0;
    }

    tex = vec4(infectedVec, tex.a);
